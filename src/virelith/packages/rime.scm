...
(define (pkg-config-alias-phases phases aliases)
  "Return a modify-phases gexp for PHASES that additionally symlinks ALIASES
into a local pkg-config directory before configure.

ALIASES is a list of (NAME . PC-FILE-PATH) pairs, as produced by
plugin-pkg-config-aliases.  The local directory is prepended to
PKG_CONFIG_PATH so the build system can find the aliased .pc names."
           ...
           (let ((pkg-config-aliases (plugin-pkg-config-aliases plugins)))
             (substitute-keyword-arguments (package-arguments librime)
                                           ...
                                           ((#:phases phases #~%standard-phases)
                                            ...
                                            ;; additionally copy Guix's own librime-lua
                                            ;; plugins/lua, producing a duplicate
                                            ;; rime-lua-objs target at configure time
                                            ;; (cmake 3.31+ treats duplicate targets
                                            ;; as an error).
                                            (if (null? pkg-config-aliases)
                                              #~(modify-phases #$phases
                                                               (delete 'install-plugins))
                                              (pkg-config-alias-phases
                                               phases
                                               pkg-config-aliases))))))
          (description
     ...
     (list
      ...
      #~(modify-phases %standard-phases
                       (add-after 'unpack 'prepare-shared-data
                                  (lambda _
                                    ;; Follow the nixpkgs/AUR convention: do not
                                    ;; let rime-ice's default.yaml replace the
                                    ;; frontend's global default.yaml.
                                    (when (file-exists? "default.yaml")
                                      (rename-file "default.yaml"
                                                   "rime_ice_suggestion.yaml"))

                                    ;; Keep runtime-relevant shared data
                                    ;; (including upstream's tracked build/
                                    ;; artifacts), but omit documentation/CI
                                    ;; files.
                                    (for-each
                                     (lambda (directory)
...
(define-public rime-data-wanxiang
  (package
    ...
    (version %rime-data-wanxiang-version)
    (source
     (origin
       (method url-fetch)
       (uri
       ...
       (sha256 %rime-data-wanxiang-sha256)))
    (build-system copy-build-system)
    (arguments
     (list
